...
export async function middleware(req) {
  ...
  if (req.url.includes("/auth") && verifiedToken) {
    return NextResponse.redirect(new URL(`/dashboard/${getDashboardByRole(role)}/info-profile`, req.url))
  }
  ...
  if (verifiedToken && req.url.includes("admin-profile") && role !== UserRoles.ADMINISTRADOR) {
    return NextResponse.redirect(new URL(`/dashboard/${getDashboardByRole(role)}/info-profile`, req.url))
  }
  ...
  if (verifiedToken && req.url.includes("customer-profile") && role !== UserRoles.TURISTA) {
    return NextResponse.redirect(new URL(`/dashboard/${getDashboardByRole(role)}/info-profile`, req.url))
  }
  ...
  if (verifiedToken && req.url.includes("/category/recomendations") && role !== UserRoles.TURISTA) {
