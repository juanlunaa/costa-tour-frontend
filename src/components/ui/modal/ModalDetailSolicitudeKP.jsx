...
export default function ModalDetailSolicitudeKP({ title, review, read }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger
        onClick={read}
        className="bg-[#F4B46A] w-[22%] rounded-full h-8 shadow-customBoxShadow hover:bg-orange-700"
      >
        Ver Detalle
      </DialogTrigger>
      <DialogContent className="bg-white h-[300px]">
        <DialogHeader className="flex justify-center items-center ">
          <DialogTitle className="font-volkhov font-bold text-4xl mb-5">
            Estados{" "}
            <span className={`font-volkhov ${titleColor}`}>{title}</span>
          </DialogTitle>
        ...
        <DialogFooter className="flex sm:justify-center items-center">
          <Button
            className="bg-[#F4B46A] w-1/2 rounded-full text-black text-xl hover:bg-orange-700 shadow-customBoxShadow "
            onClick={() => {
              setIsOpen(false)
            }}
          >
