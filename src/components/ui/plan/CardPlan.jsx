...
export const CardPlan = ({ id, nombre, miniatura, descripcion }) => {
  ...
  const descripcionRecortada = descripcion.length < 100
    ? descripcion
  ...
  return (
    <div className="w-80 flex flex-col items-center mx-auto">
      <div className="relative w-80 h-72">
        ...
        <span className="absolute top-2 left-2 py-1 px-3 bg-[#2D3134] text-white text-xs rounded-full">4.0</span>
        {isLoggedIn && (
          <button
            onClick={handleClickBookmark}
            className="absolute top-2 right-2 text-xl rounded-full text-black bg-white py-2 px-3"
          >
            {isSaved ? <FaBookmark  className="text-customBlue" /> : <FaRegBookmark />}
          </button>
        )}
      </div>
