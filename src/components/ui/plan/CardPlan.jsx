...
export const CardPlan = ({ nombre, miniatura, descripcion }) => {
  ...
  const descripcionRecortada =
    descripcion.length < longitudMaxima ? descripcion : `${descripcion.substring(0, longitudMaxima)}...`

  return (
    <div className="w-80 flex flex-col items-center mx-auto">
      <div className="relative w-80 h-72">
        <Image src={urlImage} fill className="object-cover rounded-xl" alt={nombre} />
        <h2 className="absolute bottom-2 left-2 text-white">{nombre}</h2>
        <span className="absolute top-2 left-2 py-1 px-3 text-white text-xs bg-[#2D3134] rounded-full">4.0</span>
